...
public interface FriendRecentRepository extends JpaRepository<FriendRecentEntity, Integer> {
    ...
    @Query(value = "select t from t_friend_recent t where t.user=?1 order by t.sendDate desc")
    Page<FriendRecentEntity> findByUser(UserEntity user, Pageable pageable);

    @Modifying
    @Query(value = "delete from t_friend_recent t where t.user=?1 and t.to=?2")
