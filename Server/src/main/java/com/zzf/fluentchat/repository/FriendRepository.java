...
public interface FriendRepository extends JpaRepository<FriendEntity, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "delete from friend_info t where t.user = ?1")
    void deleteAllFriend(UserEntity user);

    @Query(value = "select t1 from friend_info t1 where t1.user = ?1 and t1.state = 0 and 0 in (select t2.state from friend_info t2 where t2.user = t1.friend and t2.friend = ?1)")
